...
verify_P_is_working <- function(x, knots){
  result <- tryCatch({
    ns(x, knots = knots, intercept = FALSE) # attempt
...
create_dyadic_P_splines <- function(x, Z, J, degree){
  ...
  if (as.integer(n_dyadic)!=n_dyadic){
    ...
    if (bool == TRUE){
      return(ns(x, knots = knots, intercept = FALSE))}
    else{
...
#}
#J_val <- c(4, 8, 16, 32) 
#optimization_CV_M(simul$Z,simul$W,simul$Y,J_val,0.5,3, seq(-2, 2, by = 0.1)) #fonctionne
